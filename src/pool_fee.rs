...
impl PoolFee {
    ...
    pub fn set(&mut self, fee_repr: FeeRepr) -> Result<(), PoolError> {
        self.0 = if fee_repr.value > 0 {
            if fee_repr.value
                / (10 as ValueT)
                    .checked_pow(fee_repr.decimals as u32)
                    .ok_or(PoolError::InvalidFeeInput)?
                > 0
            {
    ...
    pub fn apply(&self, amount: u64) -> u64 {
        amount
            .mul_div_round(self.0 as u64, DECIMALS_DENOMINATOR as u64)
            .unwrap()
    }
}
...
mod tests {
    ...
    fn invalid_set_pool_fee() {
        // 5% fee
        let mut fee = PoolFee::new(FeeRepr {
            value: 500000,
            decimals: 7,
        })
        .unwrap();
